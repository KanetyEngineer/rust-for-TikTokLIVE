...
* **ローカルWeb UI**: デバッグ用のテストボタンを備えたブラウザ画面（環境設定は `config.json` を直接編集）。
...
`config.json` をテキストエディタで直接開き、以下の項目を編集して保存してください。

```json
{
    "tiktokId": "配信を行うTikTokのID（@以降）",
    "steamId": "イベントの対象となるプレイヤーの64bit Steam ID",
    "apiUrl": "http://<中央APIサーバーのIP>:5000/api/event",
    "apiToken": "管理者から共有されたapiToken"
}
```

保存後、`start.bat`（起動中の場合は再起動）を実行すると設定が反映されます。
